...
9. **The champion is the posterior-mean argmax; the card that leads is the P(best) group
   leader** -- DONE 2026-09-06 (the readout half). The published palette follows the mean
   (risk-neutral); the shelf follows P(best). **Decided: the applier keeps receiving the
   mean**, on a plateau included -- it is the risk-neutral choice over everything still
   uncertain, and switching the applier to P(best) would make what he lives in jump between
   sittings as mass moves across near-identical pages.
   At 336 responses they differ on BOTH polarities, not just night as this item used to say
   (day champion ground #faebdc against leader #f9ecde, night #1f2226 against #222427), so
   the coincidence by day was a fact about a smaller log rather than a property of day.
   `Verdict.leader`, `.leader_theme`, `.leader_stratum` and `.champion_is_leader` name the
   two apart; `pixi run verdict` prints the leader's own card with its group probability
   when they part, and the notebook says the same in prose.
   Two conflations fixed on the way, both the same shape as the `_lead_i` bug: the origin
   line printed the CHAMPION's stratum labelled "leader is", and the notebook's legibility
   sentence read "The leader reads in about N s" over the champion's number.
   (`measured-theme.json` itself is resolved: derived, so untracked, regenerated by
   `pixi run publish`.)
10. **Smaller, measured, mostly done** -- 2026-09-06.
    **The permutation test is batched** (DONE): it was 200 permutations x 2 seeds x 5 folds
    x 60 Newton steps, each an 11x11 `np.linalg.solve`, inside two Python loops. The
    permutations differ only in their tilt columns, so the Newton iteration now runs on
    stacked arrays and each step is one batched LAPACK call. Characterized against the
    unbatched implementation over both factors, both polarities and three (nperm, seed)
    settings: **identical p-values in every case**, worst difference in the null itself
    2.2e-16 (one ulp). 3.1x to 5.5x faster -- `factor_effect` 1.5 s to 0.27-0.48 s, and the
    whole suite 75 s to 66 s. The Hessian is written as a batched `(F * w)^T F` product
    rather than a three-operand einsum, which would have left BLAS for numpy's own loop.
    **Both caches are bounded** (DONE): `REALIZE_CACHE` and `PRIOR_CACHE` grew without
    limit in a server that stays up for days -- measured at about 305 and 289 entries per
    trial on top of the 2048 the standing pool realizes at import, roughly 1.4 kB an entry,
    so a 500-trial sitting added about 210 MB. Both are now `space._LruCache` at
    `CACHE_ENTRIES` = 32768 (~45 MB), holding the pool plus about a hundred trials.
    **Least-recently-USED, not insert-order**: the pool is realized once and re-read every
    trial, so a FIFO bound would evict exactly what the search depends on and hand back the
    three-times-slower no-cache behaviour. Both properties are mutation-checked, and the
    pool test counts REALIZATIONS rather than inspecting the cache afterwards -- an evicted
    entry is silently recomputed and re-inserted, so an end-state assertion passes under
    FIFO too, which is how the first version of that test was a gap.
    **`theta_key` rounding stays** (~18 ms of a warm trial): no measurement says otherwise,
    and a coarser key risks collapsing two candidates that render differently.
    The suite has no parallelism and needs none: 66 s alone; the ten-minute runs of 4 Sep
    were CPU contention from parallel agents. None of this is a GPU candidate: the operands
    are tiny and a kernel launch costs more than the work.
